(function() {
  ...
  var default_options = {
    ...
    imagePathPNG: emojione.imagePathPNG,
    filters: {
      people: {
        icon: "yum",
        names: "grinning,grin,joy,smiley,smile,sweat_smile,laughing,innocent,smiling_imp,imp,wink,blush," +
      ...
      nature: {
        icon: "whale",
        names: "seedling,evergreen_tree,deciduous_tree,palm_tree,cactus,tulip,cherry_blossom,rose,hibiscus," +
      ...
      food_drink: {
        icon: "cherries",
        names: "tomato,eggplant,corn,sweet_potato,hot_pepper,grapes,melon,watermelon,tangerine,lemon," +
      ...
      celebration: {
        icon: "tada",
        names: "ribbon,gift,birthday,jack_o_lantern,christmas_tree,tanabata_tree,bamboo,rice_scene," +
      ...
      activity: {
        icon: "trophy",
        names: "runner,walking,dancer,lifter,golfer,rowboat,swimmer,surfer,bath,snowboarder,ski," +
      ...
      travel: {
        icon: "rocket",
        names: "railway_car,mountain_railway,steam_locomotive,train,monorail,bullettrain_side," +
      ...
      objects_symbols: {
        icon: "paperclips",
        names: "watch,iphone,calling,computer,desktop,keyboard,trackball,printer,alarm_clock,clock," +
      ...
      flags: {
        ...
        names: "au,at,be,br,ca,flag_cl,cn,co,dk,fi,fr,de,hk,in,flag_id,ie,il,it,jp,kr,mo," +
  ...
  var nameToShortname = function(name) {
    return [':', name, ':'].join('');
  };
  ...
  var EmojiSelector = function(element, options) {
    ...
    this.SELECTORS = {
      FILTER_HOLDER: new Cls('emojiselector-filters'),
      FILTER: new Cls('emojiselector-filter'),
      TAB_HOLDER: new Cls('emojiselector-tabs'),
      TAB: new Cls('emojiselector-tab'),
      TAB_ID_PREFIX: new Cls('emojiselector-tab-'),
      ACTIVE: new Cls('active'),
    };
  ...
  EmojiSelector.prototype.getUnicodeString = function(shortname) {
    var entryObj = emojione.emojioneList[shortname];
    var entry = entryObj[entryObj.length-1];
    if (this.options.namesUppercase) {
      entry = entry.toUpperCase();
    }
    return entry;
  };

  EmojiSelector.prototype.createEmojiButton = function(item, attacheEvent) {
    var _this = this;
    var button = document.createElement('i');
    addClass(button, 'emojibtn');
    button.setAttribute('role', 'button');
    var emoji;
    if (this.options.sprite) {
      emoji = document.createElement('i');
      addClass(emoji, 'emojione-' + item.uni);
    } else {
      var path = [this.options.imagePathPNG, item.uni, '.png'];
      emoji = document.createElement('img');
      emoji.setAttribute('src', path.join(''))
    }

    button.appendChild(emoji);

    if (attacheEvent) {
      button.addEventListener('click', function(event, data) {
        if (_this.options.target) {
          var newValue = [_this.options.target.value, item.shortname];
          if (_this.options.additionalSpace) {
            newValue.push(' ');
          }
          _this.options.target.value = newValue.join('');
        }
      }, false);
    }
  ...
  EmojiSelector.prototype.fillTabAsync = function(tabElement, names) {
    var _this = this;
    var stack = names.slice();
    var intervalId = setInterval(function() {
      for (var i = 0; i < 20 && stack.length; i++) {
        var name = stack.shift();
        var shortname =  nameToShortname(name);
        tabElement.appendChild(_this.createEmojiButton({
          name: name,
          shortname: shortname,
          uni: _this.getUnicodeString(shortname)
        }, true));
        if (!names.length) {
  ...
  EmojiSelector.prototype.init = function() {

    var _this = this;
    for(var key in this.options.filters) { 
      if (this.options.filters.hasOwnProperty(key)) {

        var params = this.options.filters[key];
        var category = key.toString();

        var filterName = params.icon;
        var filterShortname = nameToShortname(filterName);
        var filterUni = this.getUnicodeString(filterShortname);

        // create filter
        var filterElement = document.createElement('i');
        addClass(filterElement, this.SELECTORS.FILTER);
        filterElement.setAttribute('data-filter', category);
        filterElement.appendChild(_this.createEmojiButton({
          name: filterName,
          shortname: filterShortname,
          uni: filterUni
        }));

        filterElement.addEventListener('click', function(event, data) {
          var clickedTab = (hasClass(event.target, _this.SELECTORS.FILTER)) ? event.target : closest(event.target, _this.SELECTORS.FILTER.class());
          var activeFilter = _this.element.querySelector(_this.SELECTORS.FILTER.class() + _this.SELECTORS.ACTIVE.class());
          removeClass(activeFilter, _this.SELECTORS.ACTIVE);
          addClass(clickedTab, _this.SELECTORS.ACTIVE);
          var activeTab = _this.element.querySelector(_this.SELECTORS.TAB.class() + _this.SELECTORS.ACTIVE.class());
          removeClass(activeTab, _this.SELECTORS.ACTIVE);
          var tabFilter = clickedTab.getAttribute('data-filter');
          addClass(_this.element.querySelector(_this.SELECTORS.TAB_ID_PREFIX.class() + tabFilter), _this.SELECTORS.ACTIVE);
        }, false);

        this.element.querySelector(this.SELECTORS.FILTER_HOLDER.class()).appendChild(filterElement);

        var tabElement = document.createElement('div');
        addClass(tabElement, this.SELECTORS.TAB);
        addClass(tabElement, this.SELECTORS.TAB_ID_PREFIX + category);
        var names = params.names.split(',');
        
        if (this.options.sprite) {
          for (var i in names) {
            var shortname =  nameToShortname(names[i]);
            tabElement.appendChild(this.createEmojiButton({
              name: names[i],
              shortname: shortname,
              uni: this.getUnicodeString(shortname)
            }, true));
          }
        } else {
          this.fillTabAsync(tabElement, names);
        }
        this.element.querySelector(this.SELECTORS.TAB_HOLDER.class()).appendChild(tabElement);
      }
    }

    // set first tab active
    addClass(this.element.querySelector(this.SELECTORS.FILTER.class()), this.SELECTORS.ACTIVE);
    addClass(this.element.querySelector(this.SELECTORS.TAB.class()), this.SELECTORS.ACTIVE);
